...
'''
参考：https://github.com/erelcan/keras_bahdanau.git

这个类被我自己否定掉了！！！

本来看到这个是基于RNN（keras的最新的GRU实现，之前的Recurrent父类方式已经被废弃掉了），很高兴。

但是随着代码修改，发现这个实现，只能支持Sequetial方式。

本来对于seq2seq+attention，我总是科班的觉得，解码器应该有个输入，但是为什么可以没有输入呢？
只有一个初始状态，然后解码，直到ETX，或者到达一个固定长度，就结束？！
这个思路很诡异，但是也确实说不出啥问题，所以，这个哥们的实现理论上是对的，这个哥们的代码就是这么实现的。
如何从代码中看出这个哥们是这种思路的？
是因为他用的是sequential的方式，这个就是上一个层的输出直接是下一个层的输入，
但是如果是解码器有输入的话，就不可能用sequential方式，必须要用functional方式，写成：
```
    train_model = Model(inputs=[input_image, **decoder_inputs**], outputs=decoder_pred)
``
这个decoder_inputs，就是训练的时候的标签，需要半截灌入。
这种方式，就不得不使用functional方式了。

唉，本来看到有人基于RNN实现，挺激动的，但是不支持functional方式，就很郁闷了，暂时放弃了！！！
'''
class BahdanauGRUCell(Layer):
    ...
    def build(self, input_shape):
        ...
        # *************************************************
        self.attention_kernel = self.add_weight(shape=(feature_dim_length, self.units * 3),
    ...
    def compute_step_independent_part_of_eij(self, annotations):
        logger.debug("计算能量函数结果eij:%r",annotations)
...
class BahdanauGRU(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        # bugfix:程序有bug,inputs经过父类的__call__的调用，会将inputs和initial_state合并成一个数组，导致后续的问题
        # /Users/piginzoo/software/python3/lib/python3.6/site-packages/keras/layers/recurrent.py +570 : output = super(RNN, self).__call__(full_input, **kwargs)
