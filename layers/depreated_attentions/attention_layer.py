...
class AttentionDecoder(Recurrent):
    ...
    def __init__(self, units, alphabet_size,
                 embedding_dim=30,
                 is_monotonic=False,
                 normalize_energy=False,#<------ True：Luong-style monotonic attention，False：Bahdanau-style attention (non-monotonic)
                 activation='tanh',
                 dropout=None,
        ...
        self.is_monotonic = is_monotonic #if True - Luong-style monotonic attention,if False - Bahdanau-style attention (non-monotonic)
    ...
    # use_teacher_forcing意味着训练的时候，用外部输出作为输入，而不是用上一个时刻的解码结果
    ...
    def get_initial_state(self, inputs):
        ...
        initial_states = [y0, s0, t0] # [STX,Encoder,0]

        if self.is_monotonic:# Lusong注意力
    ...
    def step(self, x, states):
        # True：Luong-style monotonic attention；False：Bahdanau-style attention (non-monotonic)//monotonic:单调的；无变化的
        if self.is_monotonic: # luSong
        ...
        if self.return_probabilities:
            ...
            output = yt # yt是返回的解码器的softma输出

        # st是新的隐状态
        next_states = [ys, st, timestep + 1]

        if self.is_monotonic: # True：Luong-style monotonic attention；False：Bahdanau-style attention (non-monotonic)//monotonic:单调的；无变化的
